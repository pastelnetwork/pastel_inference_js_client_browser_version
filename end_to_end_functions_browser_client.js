// end_to_end_functions_browser_client.js
...
async function checkForNewIncomingMessages() {
  try {
    ...
    const closestSupernodesToLocal = await utils.getNClosestSupernodesToPastelIDURLs(
      3,
...
async function sendMessageAndCheckForNewIncomingMessages(
  ...
  try {
    ...
    const closestSupernodesToRecipient = await utils.getNClosestSupernodesToPastelIDURLs(
      3,
      toPastelID,
      validMasternodeListFullDF
    );
    ...
    logger.info(`Sent user messages: ${utils.safeStringify(sendResults)}`);
...
async function handleCreditPackTicketEndToEnd(
  ...
  try {
    ...
    const closestSupernodes = await utils.getNClosestSupernodesToPastelIDURLs(12, pastelID, validMasternodeListFullDF);
    ...
    for (const supernode of closestSupernodes) {
      try {
        ...
        if (signedCreditPackTicketOrRejection.termination_reason_string) {
        ...
        await db.addData('CreditPackPurchaseRequestConfirmation', creditPackPurchaseRequestConfirmation);
...
async function buildCreditPackPurchaseRequestConfirmation(creditPackRequest, signedCreditPackTicket, burnTransactionTxid, pastelID, passphrase) {
  ...
  const { error: confirmationValidationError } = await schemas.creditPackPurchaseRequestConfirmationSchema.validate(confirmation);
...
async function initiateStorageRetry(inferenceClient, creditPackRequest, signedCreditPackTicket, validMasternodeListFullDF, pastelID, passphrase) {
  const closestAgreeingSupernodePastelID = await utils.getClosestSupernodePastelIDFromList(
    pastelID,
  ...
  await db.addData('CreditPackStorageRetryRequest', creditPackStorageRetryRequest);
  ...
  const { error: storageRetryResponseValidationError } = await schemas.creditPackStorageRetryRequestResponseSchema.validate(
  ...
  const announcementPromises = signedCreditPackTicket.list_of_supernode_pastelids_agreeing_to_credit_pack_purchase_terms.map(
    async (supernodePastelID) => {
      if (utils.checkIfPastelIDIsValid(supernodePastelID)) {
        try {
          const supernodeURL = await utils.getSupernodeUrlFromPastelID(supernodePastelID, validMasternodeListFullDF);
...
async function buildCreditPackStorageRetryRequest(creditPackRequest, signedCreditPackTicket, closestAgreeingSupernodePastelID, pastelID, passphrase) {
  const storageRetryRequest = {
    ...
    credit_pack_storage_retry_request_pastel_block_height: await rpc.getCurrentPastelBlockHeight(),
  ...
  const { error: storageRetryRequestValidationError } = await schemas.creditPackStorageRetryRequestSchema.validate(storageRetryRequest);
...
async function handleInferenceRequestEndToEnd(
  ...
  try {
    ...
    const inferenceClient = new PastelInferenceClient(pastelID, passphrase);
    const modelParametersJSON = utils.safeStringify(modelParameters);
    ...
    for (let i = 0; i < maxTries; i++) {
      ...
      try {
        const modelInputDataJSONBase64Encoded = btoa(JSON.stringify(modelInputData));
        ...
        const sha3256HashOfInferenceRequestFields =
          await utils.computeSHA3256HashOfSQLModelResponseFields(inferenceRequestData);
        ...
        const validationErrors = await utils.validateCreditPackTicketMessageData(
        ...
        if (proposedCostInCredits <= maximumInferenceCostInCredits) {
          ...
          if (txidLooksValid) {
            ...
            logger.info(
              `Sent inference confirmation: ${utils.prettyJSON(confirmationResult)}`
            );
            ...
            for (let cnt = 0; cnt < maxTriesToGetConfirmation; cnt++) {
              ...
              if (resultsAvailable) {
                ...
                if (modelInferenceTypeString === "text_to_image") {
                  let jsonString = atob(outputResults.inference_result_json_base64);
                  let jsonObject = JSON.parse(jsonString);
                  let imageBase64 = jsonObject.image;
                  inferenceResultDict.generated_image_decoded = atob(imageBase64);
                } else if (modelInferenceTypeString === "embedding_document") {
                  const inferenceResultDecoded = atob(outputResults.inference_result_json_base64);
